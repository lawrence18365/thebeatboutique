// Generates a true-301 redirect set, because the PRODUCTION origin is GitHub Pages
// (custom domain thebeatboutique.ie, served from main), which has NO server-side
// redirect support — `_redirects` and Cloudflare Pages Functions are ignored there;
// only meta-refresh stub HTML works. Cloudflare proxies the domain's DNS in front,
// so Cloudflare Bulk Redirects / Redirect Rules DO run at the edge before the GitHub
// Pages origin and produce true 301s (see output/REDIRECTS-README.md for the full
// topology and the two fix options). The functions/_middleware.js this script also
// emits is the redirect layer IF the domain is moved to the Cloudflare Pages
// deployment (verified working on the *.pages.dev preview).
//
...
// Also emit a Cloudflare Pages Functions middleware that performs the SAME 301s in
// code. This is the repo-side fix (no dashboard) for the fact that _redirects is not
// honored by this deployment. Keyed by no-trailing-slash path; exact-match only.
const fnMap = {};
for (const [from] of map.entries()) {
  if (from === '/') continue;
  fnMap[from.replace(/\/$/, '')] = resolve(from);
}
const FN_DIR = path.join(ROOT, 'functions');
fs.mkdirSync(FN_DIR, { recursive: true });
const middleware = `// AUTO-GENERATED by scripts/generate-cloudflare-redirects.js — do not edit by hand.
...
const WWW_HOST = ${JSON.stringify('www.' + HOST.replace(/^https?:\/\//, ''))};
const REDIRECTS = ${JSON.stringify(fnMap, null, 2)};
...
`;
fs.writeFileSync(path.join(FN_DIR, '_middleware.js'), middleware);
console.log(`Wrote functions/_middleware.js with ${Object.keys(fnMap).length} exact-match 301s`);
